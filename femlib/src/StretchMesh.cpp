...
//  }
  ...
  for(unsigned int ii = 0; ii<em.e.size(); ii++){
    ...
    if(opts.label[ii] == targetLabel && v2[0]<opts.xmin){
      ...
      float dx_right = steps * (float)linearScale;
      float dx_left =  (steps+1)    * (float)linearScale;
      for(int jj = 0; jj<em.e[ii]->nV(); jj++){
        int vidx = em.e[ii]->at(jj);
        if(stretched[vidx]){
          continue;
...
segmentJumper(const ElementMesh & em, StretchOpts & opts)
{
  ...
  float thresh=0.5f*gridlen + opts.beamwidth;
...
//  std::cout<<"horizontal range "<<xminIdx<<" "<<xmaxIdx<<"\n";
  ...
  opts.xmin = 0.5f * (float)(em.X[em.e[eidx]->at(2)][0] + em.X[em.e[eidx]->at(0)][0]);
  eidx = grid[xmaxIdx][topj];
  opts.xmax = 0.5f * (float)(em.X[em.e[eidx]->at(2)][0] + em.X[em.e[eidx]->at(0)][0]);
...
//  std::cout<<"number of stretched elements " << opts.nMidEle<<"\n";
  ...
  opts.beamy0 = (float)(em.X[em.e[eidx]->at(0)][1]);
...
{
  //assume input mesh em has uniform grid.
  float gridlen = (float)(em.X[em.e[0]->at(4)][0] - em.X[em.e[0]->at(0)][0]);
...
//  std::cout<<opts.nMidEle<<" "<<opts.xmin<<" "<<opts.xmax<<"\n";
  ...
  for(unsigned int ii = 0; ii<em.e.size(); ii++){
    ...
    if(v2[0]<opts.xmin){
      ...
      float dx_right = (steps+1) * (float)linearScale;
      float dx_left =   steps    * (float)linearScale;
      for(int jj = 0; jj<em.e[ii]->nV(); jj++){
        int vidx = em.e[ii]->at(jj);
        if(stretched[vidx]){
          continue;

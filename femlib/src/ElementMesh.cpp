...
void ElementMesh::scaleMesh(float scale)
{
  for(unsigned int ii = 0 ; ii<X.size(); ii++){
    X[ii] *= scale;
  }
  x=X;
  for(unsigned int ii = 0; ii<depth.size(); ii++){
    depth[ii] *= scale;
  }
}

void ElementMesh::scaleMesh(const std::vector<float> & scale)
{
  for (unsigned int ii = 0; ii<X.size(); ii++){
    for (int j = 0; j < dim; j++){
      X[ii][j] *= scale[j];
...
void ElementMesh::initArrays()
{
  ...
  color = std::vector<Eigen::Vector3f>(e.size(), Eigen::Vector3f::Ones());
...
//  std::cout<<"Compute mass...\n";
  ...
  for(unsigned int ii = 0;ii<e.size();ii++){
    ...
    double eleMass = vol*m[me[ii]]->density * depth[ii];
    totalVol  += (float)(vol*depth[ii]);
    totalMass += (float)(eleMass);
    Eigen::MatrixXd M0 = m[me[ii]]->getMassMatrix(ii, this);
